...
public class GobangGameHandler {
    ...
    private static final Logger gobangGameHandlerLogger = Logger.getLogger(GobangGameHandler.class);
    ...
    public void playerMove(int row, int col, int playerType) {
        gobangGameHandlerLogger.debug("Player " + playerType + " placed a move at (" + row + ", " + col + ")");
        ...
        gobangGameHandlerLogger.trace("Placed a move, isBlackTurn: " + game.isBlackTurn());

        // 2. 检查游戏是否结束
        if (checkWin(row, col)) {
            gobangGameHandlerLogger.info("Game ended with player " + playerType + "winning");
            game.getUi().showWinMessage(playerType);
            game.setGameOver(true);
        } else if (checkDraw()) {
            gobangGameHandlerLogger.info("Game ended with no player winning (tie)");
            game.getUi().showDrawMessage();
            game.setGameOver(true);
        } else {
            // 3. 切换回合
            game.setBlackTurn(!game.isBlackTurn());
            gobangGameHandlerLogger.trace("Switched round, isBlackTurn: " + game.isBlackTurn());

            // 4. 如果是人机对战，且游戏未结束，让AI落子
            if (game.getGameMode() == 1 && !game.isGameOver()) {
                gobangGameHandlerLogger.info("AI mode, preparing AI round...");
                aiTurn();
            }
        }
    }
